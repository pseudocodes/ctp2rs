...
| ctp-sopt v3.7.5 | sopt_v3_7_5 | x     |       | x       |
...
```toml
[dependencies]
ctp2rs = { version = "0.1", default-features = false, features = ["v1alpha1", "ctp_v6_7_7"] }
```
或者
```toml
[dependencies]
ctp2rs = { version = "0.1.10-alpha3", default-features = false, features = ["v1alpha1", "ctp_v6_7_11"] }
```
...
```shell
> CTP_API_INCLUDE_DIR=/absolute/path/to/your/ctp/api/ \
cargo build --no-default-features --features "v1alpha1,dynlib,union"
```
...
| 12  |    rn7s2/ctp-rs    | [https://github.com/rn7s2/ctp-rs](https://github.com/rn7s2/ctp-rs)                                 |       [ctp-rs](https://crates.io/crates/ctp-rs)       | Win64/Linux/MacOS  | 原名 `ctp4rs` 后占据 `ctp-rs` crate, 高度定制化的静态 C++ 代码辅助封装，采用了 `cxx-build` 作为 C++ 代码桥接工具, 仅支持 ctp 最新版本, [2026-05] 之后添加 MacOS/localctp 支持 |
